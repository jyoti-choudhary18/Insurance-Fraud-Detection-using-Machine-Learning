...
class File_Operation:
    ...
    def save_model(self, model, filename):
        ...
        try:
            ...
            # CRITICAL FIX: Only remove the specific model folder (path), NOT the whole directory (self.model_directory)
            if os.path.isdir(path): 
                shutil.rmtree(path)  # <--- CHANGED FROM self.model_directory TO path
                os.makedirs(path)
            else:
                os.makedirs(path)
