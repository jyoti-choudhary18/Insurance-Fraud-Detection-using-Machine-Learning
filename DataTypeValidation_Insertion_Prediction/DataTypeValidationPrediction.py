        ...
        for file in onlyfiles:
            try:
                with open(goodFilePath+'/'+file, "r") as f:
                    ...
                    # FIX: Don't use csv.reader with \n. Just read the file line by line.
                    for line in f:
                        try:
                            # line.strip() removes the newline character at the end
                            # We check if line is not empty to avoid insertion errors
                            if not line.strip(): 
                                continue
                            conn.execute('INSERT INTO Good_Raw_Data values ({values})'.format(values=(line.strip())))
