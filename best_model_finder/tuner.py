...
class Model_Finder:
    ...
    def get_best_params_for_xgboost(self,train_x,train_y):
        ...
        try:
            # initializing with different combination of parameters
            # FIX: Removed "criterion" because XGBoost does not use gini/entropy (those are for Random Forest)
            self.param_grid_xgboost = {
                "n_estimators": [100, 130], 
                "max_depth": range(8, 10, 1)
            }
            # Creating an object of the Grid Search class
            self.grid= GridSearchCV(XGBClassifier(objective='binary:logistic', n_jobs=-1),self.param_grid_xgboost, verbose=3,cv=5)
            # finding the best parameters
            self.grid.fit(train_x, train_y)
    ...
    def get_best_model(self,train_x,train_y,test_x,test_y):
        ...
        try:
            ...
            if len(test_y.unique()) == 1:#if there is only one label in y, then roc_auc_score returns error. We will use accuracy in that case
                ...
                # FIX: Changed self.sv_score to self.svm_score
