import pandas as pd
import numpy as np
import pickle # Needed to save the scaler
...
class Preprocessor:
    ...
    Version: 1.2 (Fixed Scaling Logic)
    """
    ...
    def is_null_present(self,data):
        ...
        try:
            ...
            self.logger_object.log(self.file_object,'Finding missing values success.')
            return self.null_present, self.cols_with_missing_values
        except Exception as e:
            self.logger_object.log(self.file_object,'Exception in is_null_present: ' + str(e))
            raise Exception()
    ...
    def impute_missing_values(self, data, cols_with_missing_values):
        ...
        try:
            ...
            self.data[self.cols_with_missing_values] = self.imputer.fit_transform(self.data[self.cols_with_missing_values])
            self.logger_object.log(self.file_object, 'Imputing missing values Successful.')
            return self.data
        except Exception as e:
            self.logger_object.log(self.file_object,'Exception in impute_missing_values: ' + str(e))
            raise Exception()

    # --- CHANGED: Split into two functions for correct Scaling ---
    
    def fit_scale_numerical_columns(self, data):
        """ Used ONLY during TRAINING to Learn and Save the scaling """
        self.logger_object.log(self.file_object, 'Entered the fit_scale_numerical_columns method')
        self.data = data
        ...
        try:
            ...
            with open('preprocessing_data/std_scaler.pickle', 'wb') as f:
            ...
            self.logger_object.log(self.file_object, 'Scaling trained and saved successfully.')
            return self.data
        except Exception as e:
            self.logger_object.log(self.file_object,'Exception in fit_scale: ' + str(e))
            raise Exception()

    def transform_only_numerical_columns(self, data):
        """ Used ONLY during PREDICTION to Load the scaler and use it """
        self.logger_object.log(self.file_object, 'Entered the transform_only_numerical_columns method')
        self.data = data
        self.num_df = self.data[['months_as_customer', 'policy_deductable', 'umbrella_limit',
                          'capital-gains', 'capital-loss', 'incident_hour_of_the_day',
                          'number_of_vehicles_involved', 'bodily_injuries', 'witnesses', 'injury_claim',
        ...
        try:
            ...
            with open('preprocessing_data/std_scaler.pickle', 'rb') as f:
            ...
            self.data.drop(columns=self.scaled_num_df.columns, inplace=True)
            self.data = pd.concat([self.scaled_num_df, self.data], axis=1)
            self.logger_object.log(self.file_object, 'Scaling transformed successfully using saved scaler.')
            return self.data
        except Exception as e:
            self.logger_object.log(self.file_object,'Exception in transform_only: ' + str(e))
            # Fallback: if no scaler found, just return data (risky but prevents crash)
            return self.data
    ...
    def handle_imbalanced_dataset(self,x,y):
        ...
        try:
            ...
            # FIX: Changed fit_sample to fit_resample
